        ...
        <div className="flex flex-col h-full bg-[#1a1d29]">
            ...
            <div className="flex-none p-4 md:p-6 border-b border-[#2d3142] flex flex-wrap gap-4 md:gap-6 items-center justify-between bg-[#1a1d29]/80 backdrop-blur-md sticky top-0 z-20">
                <div>
                    ...
                    <p className="text-sm text-gray-400 mt-1">Manage guest narrative, crowd flow, and rapid response.</p>
                </div>

                <div className="flex items-center gap-2 bg-[#2d3142] p-2 rounded-xl border border-gray-700">
                    <span className="text-xs md:text-sm text-gray-400 font-bold px-2 hidden sm:block">Zone {staff.current_zone_id?.replace('Z-', '') || '01'} Status:</span>
                    <div className={`px-3 md:px-4 py-1.5 md:py-2 rounded-lg font-bold text-xs md:text-sm ${zoneStatuses[staff.current_zone_id || 'Z-01'] === 'Cleaning' ? 'bg-yellow-500/20 text-yellow-500 border border-yellow-500/30' : 'bg-green-500/20 text-green-400 border border-green-500/30'}`}>
                        {zoneStatuses[staff.current_zone_id || 'Z-01'] === 'Cleaning' ? 'Cleaning' : 'Ready'}
                    </div>
                </div>
            </div>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        <div className="lg:col-span-3 grid grid-cols-1 sm:grid-cols-3 gap-4">
                            ...
                            >
                                ...
                                <span className="font-bold text-red-500 group-hover:text-white uppercase tracking-widest text-xs md:text-sm">Medical Emergency</span>
                            </button>
                            ...
                            >
                                ...
                                <span className="font-bold text-blue-500 group-hover:text-white uppercase tracking-widest text-xs md:text-sm">Security Response</span>
                            </button>
                        ...
                        <div className="col-span-1 bg-[#2d3142] p-6 rounded-2xl border border-white/5 shadow-xl flex flex-col">
                            ...
                            <div className="flex gap-2 mb-4">
                                <input
                                    ...
                                    className="flex-1 bg-[#1a1d29] border border-gray-700 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-yellow-500/50 min-w-0"
                                />
                                <button onClick={handleAddHighlight} className="bg-yellow-500/20 text-yellow-400 px-3 py-2 rounded-lg hover:bg-yellow-500/30 transition-colors whitespace-nowrap">
                                    Add
                                </button>
                            </div>

                            <div className="flex-1 overflow-y-auto space-y-3 max-h-[300px] custom-scrollbar">
                                {highlights.map((h, i) => (
